// Code generated by mockery v2.10.0. DO NOT EDIT.

package mocks

import (
	sqlx "github.com/jmoiron/sqlx"
	mock "github.com/stretchr/testify/mock"

	user "sample/twirp/rpc/user"

	userModel "sample/twirp/model/user"
)

// UserRepository is an autogenerated mock type for the UserRepository type
type UserRepository struct {
	mock.Mock
}

// FindUserByEmail provides a mock function with given fields: db, email
func (_m *UserRepository) FindUserByEmail(db sqlx.DB, email string) *userModel.User {
	ret := _m.Called(db, email)

	var r0 *userModel.User
	if rf, ok := ret.Get(0).(func(sqlx.DB, string) *userModel.User); ok {
		r0 = rf(db, email)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(*userModel.User)
		}
	}

	return r0
}

// FindUserById provides a mock function with given fields: db, id
func (_m *UserRepository) FindUserById(db sqlx.DB, id int) *userModel.User {
	ret := _m.Called(db, id)

	var r0 *userModel.User
	if rf, ok := ret.Get(0).(func(sqlx.DB, int) *userModel.User); ok {
		r0 = rf(db, id)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(*userModel.User)
		}
	}

	return r0
}

// InsertOne provides a mock function with given fields: db, _a1
func (_m *UserRepository) InsertOne(db sqlx.DB, _a1 *user.User) int64 {
	ret := _m.Called(db, _a1)

	var r0 int64
	if rf, ok := ret.Get(0).(func(sqlx.DB, *user.User) int64); ok {
		r0 = rf(db, _a1)
	} else {
		r0 = ret.Get(0).(int64)
	}

	return r0
}

// indUserByEmail provides a mock function with given fields: db, email
func (_m *UserRepository) indUserByEmail(db sqlx.DB, email string) *userModel.User {
	ret := _m.Called(db, email)

	var r0 *userModel.User
	if rf, ok := ret.Get(0).(func(sqlx.DB, string) *userModel.User); ok {
		r0 = rf(db, email)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(*userModel.User)
		}
	}

	return r0
}
